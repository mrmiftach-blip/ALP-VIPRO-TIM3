...
import com.traffic.traffic_monitoring_backend.dto.*
import com.traffic.traffic_monitoring_backend.model.TrafficCamera
import com.traffic.traffic_monitoring_backend.repository.TrafficCameraRepository
...
class TrafficCameraService(
	...
	private suspend fun fetchFromLTA(dateTime: LocalDateTime?): LtaTrafficImageResponse {
		val uriSpec = webClient.get()
			...
			.header("AccountKey", apiKey)  // LTA API menggunakan AccountKey header

		return uriSpec.retrieve()
			.bodyToMono<LtaTrafficImageResponse>()
			.awaitSingle()
	}
